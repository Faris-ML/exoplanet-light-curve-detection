...
def make_cnn(maxlen):
    ...
    SGDsolver = SGD(learning_rate=0.1, momentum=0.25, decay=0.0001, nesterov=True)
...
def generate_positives(residuals, pars, N=10000, minsnr=1.5, snrscale=2):
    ...
    for i in range(10000):
        ...
        while (tlength < 15):
            ...
            tmodel = times[di][tmask]/(pars['rp']*pars['tm']*pars['per']*pars['ar'])
...
if __name__ == "__main__":
    ...
    try:
        ...
        sv = pickle.load( open('C:\\Users\\TFgam\\OneDrive\\سطح المكتب\\exo\\Exoplanet-Artificial-Intelligence-master\\TESS\\TESS_residual.pkl','rb') )
        print(sv)
    ...
    y_pred = np.concatenate((y_pos,y_neg),axis=0)
    ...
    lr_precision, lr_recall, _ = metrics.precision_recall_curve(yt, y_pred)
    lr_f1, lr_auc = metrics.f1_score(yt, y_pred), metrics.auc(lr_recall, lr_precision)
     # summarize scores
